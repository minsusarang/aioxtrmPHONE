...
type
  TFrmMain = class(TForm)
    ...
    procedure LoginFlowTimerTimer(Sender: TObject);
  ...
  private
    ...
    FLoginAfterLogout: Boolean;
    FCallConnectedAt: TDateTime;
    FCallEndedAt: TDateTime;
    FLastCallDurationText: string;
    FDisplayRec: string;
    FDisplayAni: string;
    ...
    function BuildDialPayload(const ANumber: string): string;
    ...
    function PreparePayloadForSend(const APayload: string): string;
    function SendJson(const APayload: string): Boolean;
    procedure SendJsonAsync(const APayload: string);
    procedure HandleIncomingText(const AText: string);
    procedure ResetCallContext;
    procedure ClearCallInfoDisplay;
...
begin
  if not Assigned(FAlertPopup) then
    FAlertPopup := TAlertPopupForm.Create(nil);
  FAlertPopup.ShowAlert(AMessage);
end;
procedure TFrmMain.HideAlert;
begin
  AlertTimer.Enabled := False;
  if Assigned(FAlertPopup) then
    FAlertPopup.HideAlert;
end;
...
function TFrmMain.BuildDialPayload(const ANumber: string): string;
var
  LJson: TJSONObject;
  LCallType: string;
begin
  ...
  LCallType := Trim(FCallType);
  if SameText(LCallType, 'Idle') or (LCallType = '') then
    LCallType := Trim(FMode);
  LJson := TJSONObject.Create;
  try
    LJson.AddPair('req', 'dial');
    LJson.AddPair('userid', Trim(edtUserId.Text));
    LJson.AddPair('num', ANumber);
    LJson.AddPair('call_type', LCallType);
...
begin
  try
    if EnsureClientReady then
      SendJsonAsync(BuildAnswerPayload);
  except
    on E: Exception do
    begin
...
begin
  ...
  try
    ...
    if EnsureClientReady then
    begin
      AddLog(Format('Dial request prepared: state=%s, mode=%s, call_type=%s, num=%s',
        [FState, FMode, FCallType, Trim(edtDialNumber.Text)]));
      SendJsonAsync(BuildDialPayload(Trim(edtDialNumber.Text)));
    end;
  except
    on E: Exception do
    begin
...
begin
  try
    if EnsureClientReady then
      SendJsonAsync(BuildHangupPayload);
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
  ...
  try
    ...
    begin
      if not SameText(FMode, 'Hold') then
        FPrevHoldMode := FMode;
      SendJsonAsync(BuildToggleModePayload('hold'));
    end;
  except
    on E: Exception do
    begin
...
begin
  try
    ...
    begin
      ...
      SendJsonAsync(BuildSetModePayload('Inbound'));
    end;
  except
    on E: Exception do
    begin
...
begin
  ...
  try
    if EnsureClientReady then
      SendJsonAsync(BuildToggleModePayload('notReady'));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
begin
  ...
  try
    ...
    begin
      ...
      SendJsonAsync(BuildSetModePayload('Outbound'));
    end;
  except
    on E: Exception do
    begin
...
begin
  try
    ...
    if EnsureClientReady then
      SendJsonAsync(BuildTransferPayload(Trim(edtTransferNumber.Text)));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
begin
  try
    if Trim(edtTransferNumber.Text) = '' then
      raise Exception.Create('Enter transfer number.');
    if EnsureClientReady then
      SendJsonAsync(BuildTransferColdPayload(Trim(edtTransferNumber.Text)));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
begin
  try
    if Trim(edtTransferNumber.Text) = '' then
      raise Exception.Create('Enter transfer number.');
    if EnsureClientReady then
      SendJsonAsync(BuildTransferWarmPayload(Trim(edtTransferNumber.Text)));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
begin
  ...
  FCallConnectedAt := 0;
  FCallEndedAt := 0;
  FLastCallDurationText := '00:00:00';
  FDisplayRec := '';
  FDisplayAni := '';
  FAlertPopup := nil;
...
begin
  ...
  try
    ...
    LPrevMode := FMode;
    LPrevState := FState;
    LWasConnected := IsAnyState(['Connect', 'TxConnect', 'TxConnectC', 'Tx3Connect']);
    ...
    if SameText(LEvt, 'call') then
    begin
      if SameText(FCallType, 'Inbound') or SameText(FCallType, 'Outbound') then
      begin
        if Trim(FUid) <> '' then
          FDisplayRec := FUid;
        if Trim(FCPhone) <> '' then
          FDisplayAni := FCPhone;
      end;
    end;
    ...
    begin
      ...
      if SameText(FCallType, 'Idle') or (Trim(FCallType) = '') then
        FCallType := FMode;
      if Trim(FState) = '' then
        FState := 'Idle';
      FSession := 'None';
    ...
    begin
      if (Trim(ExtractJsonValue(LJson, 'mode')) = '') and (Trim(FPendingMode) <> '') then
        FMode := FPendingMode;
      if (Trim(ExtractJsonValue(LJson, 'call_type')) = '') and
         (SameText(FCallType, 'Idle') or (Trim(FCallType) = '')) then
        FCallType := FMode;
    ...
    begin
      if FLoginAfterLogout then
      begin
        FLoginAfterLogout := False;
        LoginFlowTimer.Enabled := False;
        AddLog('Logout cleanup acknowledged, continuing login');
    ...
    begin
      if SameText(LReq, 'setMode') then
        FPendingMode := '';
      if SameText(LReq, 'logout') and FLoginAfterLogout then
      begin
        FLoginAfterLogout := False;
        LoginFlowTimer.Enabled := False;
        AddLog('Logout cleanup failed or session already closed, continuing login');
        SendJson(BuildLoginPayload);
      end;
      LValue := ExtractJsonValue(LJson, 'msg');
      AddLog('Server error: ' + LValue);
      if Trim(LValue) <> '' then
        ShowAlert(LValue);
    end
    ...
    LIsConnected := IsAnyState(['Connect', 'TxConnect', 'TxConnectC', 'Tx3Connect']);
    if (not LWasConnected) and LIsConnected then
    begin
      FCallConnectedAt := Now;
      FCallEndedAt := 0;
      FLastCallDurationText := '00:00:00';
    end
    else if LWasConnected and (not LIsConnected) and
            (not SameText(LPrevState, FState)) then
    begin
      if FCallConnectedAt > 0 then
      begin
        FCallEndedAt := Now;
        FLastCallDurationText := FormatElapsedTime(FCallConnectedAt);
      end;
    end;
  finally
    LJson.Free;
...
procedure TFrmMain.LoginFlowTimerTimer(Sender: TObject);
begin
  LoginFlowTimer.Enabled := False;
  if not FLoginAfterLogout then
    Exit;
  FLoginAfterLogout := False;
  try
    if EnsureSocketConnected and not FLoggedIn then
    begin
      AddLog('Logout cleanup timeout, continuing login');
      SendJson(BuildLoginPayload);
    end;
  except
    on E: Exception do
...
procedure TFrmMain.AlertTimerTimer(Sender: TObject);
begin
  HideAlert;
end;
...
begin
  ...
  UpdateCallInfoFields;
end;
procedure TFrmMain.ClearCallInfoDisplay;
begin
  FCallConnectedAt := 0;
  FCallEndedAt := 0;
  FLastCallDurationText := '00:00:00';
...
procedure TFrmMain.UpdateCallInfoFields;
var
  LDurationText: string;
begin
  if FCallConnectedAt > 0 then
    edtSrtTime.Text := FormatDateTime('hh:nn:ss', FCallConnectedAt)
  else
    edtSrtTime.Text := '';
  if (FCallConnectedAt > 0) and (FCallEndedAt <= 0) and
     IsAnyState(['Connect', 'TxConnect', 'TxConnectC', 'Tx3Connect']) then
    LDurationText := FormatElapsedTime(FCallConnectedAt)
  else
    LDurationText := FLastCallDurationText;
  if Trim(LDurationText) = '' then
    LDurationText := '00:00:00';
  edtDuration.Text := LDurationText;
  if FCallEndedAt > 0 then
    edtEndTime.Text := FormatDateTime('hh:nn:ss', FCallEndedAt)
  else
    edtEndTime.Text := '';
  edtRec.Text := Trim(FDisplayRec);
  edtAni.Text := Trim(FDisplayAni);
  if SameText(FCallType, 'Inbound') then
    edtInOut.Text := 'Inbound'
  else if SameText(FCallType, 'Outbound') then
    edtInOut.Text := 'OutBound'
  else
    edtInOut.Text := '';
end;
...
function TFrmMain.PreparePayloadForSend(const APayload: string): string;
var
  LJson: TJSONObject;
  LText: string;
begin
...
procedure TFrmMain.SendJsonAsync(const APayload: string);
var
  LText: string;
begin
  if not (Assigned(FClient) and FClient.Active) then
    raise Exception.Create('WebSocket is not connected.');
  LText := PreparePayloadForSend(APayload);
  AddLog('SEND ' + LText);
...
begin
  try
    ...
    if LRequestName = '' then
      Exit;
    if EnsureClientReady then
      SendJsonAsync(BuildToggleModePayload(LRequestName));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
begin
  ...
  lab_Status_Time.Text := FormatElapsedTime(FStateStartedAt);
end;
...
begin
  ...
  FLoginAfterLogout := True;
  LoginFlowTimer.Enabled := False;
  AddLog('Trying logout cleanup before login');
  try
    SendJson(BuildLogoutPayload);
    LoginFlowTimer.Enabled := True;
  except
    on E: Exception do
    begin
      FLoginAfterLogout := False;
      AddLog('Logout cleanup send failed, continuing login: ' + E.Message);
      SendJson(BuildLoginPayload);
    end;
  end;
  UpdateButtons;
