var vNotify = (function() {
  ...
  var options = {
    ...
    position: positionOption.topLeft,
    sticky: false,
    showClose: true
  };
  ...
  var addNotify = function(params) {
    ...
    item.addEventListener('click', function(evt){ removeAction(item);});
  ...
  var removeAction = function(item) {
    if(item.params){
      if(typeof item.params.callback==='function'){
        console.log('vnotify: click action...');
        item.params.callback.call(item.params);
        // ex: vNotify.info({text:'aa',callback:function(arg){ console.log(arg); },cbThis:null,cbArg:[1,2]})
      }
      clearTimeout(item.interval);
      clearTimeout(item.fadeInterval);
      item.style.display = 'none';
