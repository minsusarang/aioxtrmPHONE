...
uses
  Winapi.Windows,  System.Classes,  System.IniFiles,  System.JSON,   SysUtils,
  System.UITypes,  System.Net.URLClient,  Vcl.Controls,  Vcl.Dialogs,  Vcl.ExtCtrls,
  Vcl.Forms,  Vcl.Graphics,  Vcl.StdCtrls,  CallListForm,
...
type
  TFrmMain = class(TForm)
    ...
    lblStatus: TLabel;
    Label1: TLabel;
    lab_Status_NM: TLabel;
    lab_Status_Time: TLabel;
    ...
    procedure btnMeetingClick(Sender: TObject);
    procedure btnSeminarClick(Sender: TObject);
...
procedure TFrmMain.btnAwayClick(Sender: TObject);
begin
  try
    if EnsureClientReady then
      SendJson(BuildToggleModePayload('away'));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
procedure TFrmMain.btnEtcClick(Sender: TObject);
begin
  try
    if EnsureClientReady then
      SendJson(BuildToggleModePayload('etc'));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
procedure TFrmMain.btnLunchClick(Sender: TObject);
begin
  try
    if EnsureClientReady then
      SendJson(BuildToggleModePayload('lunch'));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
procedure TFrmMain.btnMeetingClick(Sender: TObject);
begin
  try
    if EnsureClientReady then
      SendJson(BuildToggleModePayload('meeting'));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
procedure TFrmMain.btnRestClick(Sender: TObject);
begin
  try
    if EnsureClientReady then
      SendJson(BuildToggleModePayload('rest'));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
procedure TFrmMain.btnSeminarClick(Sender: TObject);
begin
  try
    if EnsureClientReady then
      SendJson(BuildToggleModePayload('seminar'));
  except
    on E: Exception do
    begin
      AddLog('ERROR ' + E.Message);
...
begin
  ...
  if Trim(edtCtiServer.Text) = '' then
    raise Exception.Create('Enter CTI server.');
  ...
  AddLog(Format('CTI server: %s / userid: %s / phoneid: %s',
    [Trim(edtCtiServer.Text), Trim(edtUserId.Text), Trim(edtPhoneId.Text)]));
...
begin
  ...
  edtCtiServer.Text := '172.28.224.230';
...
begin
  ...
  try
    edtWebSocketUrl.Text := LIni.ReadString('Connection', 'WebSocketUrl', edtWebSocketUrl.Text);
    edtCtiServer.Text := LIni.ReadString('Connection', 'CtiServer', edtCtiServer.Text);
    edtUserId.Text := LIni.ReadString('Connection', 'UserId', edtUserId.Text);
    edtPhoneId.Text := LIni.ReadString('Connection', 'PhoneId', edtPhoneId.Text);
    cbInitMode.Text := LIni.ReadString('Connection', 'InitMode', cbInitMode.Text);
    chkOverrideLogin.Checked := LIni.ReadBool('Connection', 'OverrideLogin', chkOverrideLogin.Checked);
    chkAutoReconnect.Checked := LIni.ReadBool('Connection', 'AutoReconnect', chkAutoReconnect.Checked);
...
begin
  ...
  lblTitle.Caption := 'CTI WebSocket ' + #51228#50612 + ' ' + #53080#49548;

  lblSubtitle.Font.Name := 'Segoe UI';
  lblSubtitle.Font.Size := 9;
  lblSubtitle.Font.Color := RGB(211, 219, 233);
  lblSubtitle.Caption := 'Delphi 11.3 + SGC WebSockets / Contact Center CTI Sample';
...
begin
  ...
  try
    ...
    LIni.WriteString('Connection' ,'CtiServer'     ,Trim(edtCtiServer.Text));
...
begin
  if SameText(FMode, 'Hold') then
    btnHold.Caption := #54848#46300'/'#54644#51228
  else
    btnHold.Caption := #54848#46300;
  ...
  btnLogin.Enabled := Assigned(FClient) and FClient.Active and not FLoggedIn;
  btnLogout.Enabled := Assigned(FClient) and FClient.Active and FLoggedIn;
  ...
  begin
    ...
    begin
      ...
      btnAway.Enabled := not SameText(FMode, 'Away');
      btnRest.Enabled := not SameText(FMode, 'Rest');
      btnLunch.Enabled := not SameText(FMode, 'Lunch');
      btnMeeting.Enabled := not SameText(FMode, 'Meeting');
      btnSeminar.Enabled := not SameText(FMode, 'Seminar');
      btnEtc.Enabled := not SameText(FMode, 'Etc');
