...
const
    ...
    { Length constants }
    ctmpMaxAccountInfoLen      =  32;  // 64 --> 32
    ctmpMaxAuthInfoLen         =  32;  // 2011.03.03 추가
    ctmpMaxApplLen             =  64;
    ctmpMaxAgentId             =  10;  // 10 --> 11  --> 10 //2011.03.03
    ctmpMaxDeviceLen           =  24;
    ...
    ctmpMaxTimeStamp           =  15;
    ctmpMaxAttDataLen          =  1024; // 2011.03.03   2048 --> 1024
    ctmpMaxSysDataLen          =  256;
    ctmpMaxReadyCount          =  50;

    ctmpMaxEmployeeLen				 = 	32; //* added by K.CH.S 2008.01.29 nexuscube */ 2011.03.03
    ctmpMaxLen30						   =  31; //* added by K.CH.S 2008.01.29 nexuscube */ 2011.03.03
    ctmpMaxLen20						   =  21; //* added by K.CH.S 2008.01.29 nexuscube */ 2011.03.03
...
//    ctmpF_VoiceUnitFN	                = $49;		{ 'I' }  //2011.03.03 삭제

    ctmpF_VoiceUnitFN                   = $50; //        /* 'J' */
    ctmpF_MailFN                        = $4a; //        /* 'K' */
    cubeF_HaFN                          = $54; //        /* 'T' */
    cubeF_OutboundFN                    = $55; //        /* 'U' */
    cubeF_StatisticsFN                  = $56; //        /* 'V' */
    cubeF_RoutingFN                     = $57; //        /* 'W' */
    cubeF_SystemFN                      = $58; //        /* 'X' */
    ctmpF_CallNotify                    = $59; //        /* 'Y' */
    ctmpF_LinkMsgFN                     = $5a; //        /* 'Z' */
    ...
    ctmpF_DivertNoRet                   = 84;
    ctmpF_HoldCall	                = 90;
    ctmpF_MakeCall	                = 101;
    ctmpF_MakePredictiveCall            = 102;
    ctmpF_ObservationCall               = 103;
    ctmpF_ObservationTalkCall           = 104;
    ctmpF_OneStepTransfer               = 105; // consult+Transfer
    ctmpF_OneStepConference             = 106; // Consult + Conference
    ...
    ctmpF_SingleStepConference          = 171;{ ASAI }
    ctmpF_SingleStepTransfer            = 172;{ ASAI }
    ctmpF_QueryMailAgentStatus          = 200;
    ...
    ctmpF_SetFeatureMailAgentStatus     = 220;
    ...
    ctmpF_AgentBusyGet                  = 92;  //==> Api 에서는 Switch로 바꿈
    ctmpF_AgentSkillGet                 = 93;
    ctmpF_CallToCNID			= 95; { CSTA }
    ctmpF_AgentReadyGetEx               = 96;
    ctmpF_MultiAddDevice		= 100;{ CSTA }
    ctmpF_QueryAgentStatusEx            = 110; //==> Api 에서는 Switch로 바꿈
    ctmpF_QueryAgentMailStatusEx        = 111;
    ctmpF_AttachData			= 120;{ CSTA }
    ctmpF_WaitTimeGet			= 130;{ CSTA }
//    ctmpF_WaitTimeGetEx                 = 131;  //2011.03.03 삭제
    ctmpF_SystemData			= 140;{ CSTA }
    ctmpF_AvortStream			= 150;{ ASAI }

{    ctmpF_QueryRoutePoinInfoEx          = 230; //2011.03.03 삭제
    ctmpF_QueryDNISInfo                 = 231;
    ctmpF_AgentSPPassword               = 244;
    ctmpF_PreviewCampaignInfo           = 241;
    ctmpF_PreviewCampaignData           = 242;
    ctmpF_PreviewResult                 = 243;
    ctmpF_CounselResultExMaster         = 251;
{}
    ctmpF_PwdConfirm                    = 252;

    ctmpF_CenterInfo                    = 150;
    ctmpF_AgentBusyGetII                = 151;
    ctmpF_AgentReadyGetII               = 152;
    ctmpF_AgentStatusGetII              = 153;
    ctmpF_QueryAgentStatusExII          = 154;

//    ctmpF_SqlExcute                     = 160;
    ctmpF_SetUEIData                    = 170;
    ctmpF_WaitAgentInQueueGet           = 180;
    ctmpF_AgentStatusGet                = 190;

    ctmpF_SystemEventIE                 = 199;         // added by K.CH.S 2008.01.31 nexuscube  //2011.03.03

    ctmpF_CallBackCampaign              = 200;
    ctmpF_CallBackCampaignListInsert    = 201;
    ctmpF_CounselResult                 = 202;

    ctmpF_AgentBlocking                 = 203; // 204 -> 203 //2011.03.03

    ctmpF_GetTenantMaster					      = 205; // 2011.03.03 추가
    ctmpF_GetGroupMaster					      = 206; // 2011.03.03 추가
    ctmpF_GetPartMaster						      = 207; // 2011.03.03 추가
    ctmpF_GetAgentMaster					      = 208; // 2011.03.03 추가

    ctmpF_ReadyRateGet                  = 239;
    ctmpF_SetAgentLeavingFlag           = 250; // 240 -> 250

    ctmpF_CampaignListHandle            = 245;
    ctmpF_GetMaxCPSQ                    = 246;
    ctmpF_BlackListInsert               = 247;
    ctmpF_PreviewMakeCall               = 248;

    ctmpF_CreateCampaign				        = 249; // 2011.03.03 추가
    ctmpF_PopupEvent					    	    = 254; // 2011.03.03 추가
    ctmpF_SetAgentBlocking					    = 255; // 2011.03.03 추가


    ctmpF_BlackListIIInsert             = 249;
    //ctmpF_CreateCampaign                = 250;

    ctmpF_PwdConfirmEvent               = 253;
    ctmpF_SendUserData                  = 253;
    ctmpF_MsgSend                       = 180;
    ctmpF_UsbDnStatus                   = 238;
    ...
    {// cube ...}
    cubeS_GetScenario						           = 10;
    cubeS_SetSkillToCNID					         = 20;
    cubeS_AppendWaitCallInfo			         = 30;
    cubeS_AppendSystemCallRetrieve         = 40;
    cubeS_ApplyScenarioResult			         = 50;

    cubeS_IVRRegisterCall					         = 60;
    cubeS_IVRReleaseCall					         = 61;
    cubeS_IVRAllowRouting					         = 62;
    cubeS_IVRNumberOfWait					         = 63;

    cubeS_SetAgentFlag                     = 90;
    cubeS_SQLQuery                         = 94;
    cubeS_AgentGetBySkill                  = 97;
    cubeS_AuthenticationComplete           = 98;
    cubeS_WaitCallList                     = 99;
    cubeS_GetSkillMaster                   = 210;


    cubeS_SQLExecute						           = 160;			// 2009.02.22 jwkim rename and properties change.
...
type
    ...
    ctmpINT             = u_int;
    ctmpSHORT           = u_int;// short; //20110317
    ctmpCHAR            = u_char;
    ctmpFLOAT           = float;
    ctmpGateID          = u_int;
    ctmpCenterID        = u_char;
...
{            ctmpEK_eDiverted                = 15;  //2011.03.03 박정완
// 2011.03.03 박정완
}
        ctmpEK_Route                    = 15;
...
{}
        ...
        ctmpEK_SystemEventIE            = 200;         // added by K.CH.S 2008.01.31 nexuscube //2011.03.03 박정완
	    ctmpEK_LoggedOn                 = 201;
      ...
      ctmpEK_MAfterCallWork           = 244;
        ctmpEK_MDelivery                = 245;//17;
        ctmpEK_MComplete                = 256;//16;
      ...
      ctmpEK_PwdConfirm               = 253; //252;  { 2008.07.07 skkim add ars event make }
      ctmpEK_UserData	            = 253;  { 2009.12.03	jbpark	MCS }
...
type
    ctmpSystemEventIE_Def = TOleEnum;
const
 		ctmpSE_Tenant                       = 1;
		ctmpSE_Group                        = 2;
		ctmpSE_Part                         = 3;
		...
		cubeSE_IVRPopup                     = 213;	  // NEXUSCUBE v2.1.57.2
...
type
    ctmpTempArray          = array[0.. 4096] of Char;
    ctmpAccountInfo        = array[0..ctmpMaxAccountInfoLen   -1] of  u_char;
    ctmpApplString         = array[0..ctmpMaxApplLen          -1] of  char;
    ctmpApplStringbyte     = array[0..ctmpMaxApplLen          -1] of  byte;
    ctmpDeviceString       = array[0..ctmpMaxDeviceLen        -1] of  char;
    ctmpErrorMSG           = array[0..ctmpMaxErrorMSGLen      -1] of  char;
    ctmpIpAddr             = array[0..ctmpMaxIPAddrLen        -1] of  char;
    ctmpLogIdString        = array[0..ctmpMaxLogIDLen         -1] of  char;
    ctmpManufacturerString = array[0..ctmpMaxManufacturerLen  -1] of  char;
    ctmpNameString         = array[0..ctmpMaxNameLen          -1] of  char;
    ctmpNetString          = array[0..ctmpMaxNetLen           -1] of  char;
    ctmpPrivateString      = array[0..ctmpMaxPrivateRawDataLen-1] of  char;
    ctmpIOData             = array[0..ctmpMaxErrorMSGLen      -1] of  char;
    ctmpDataString         = array[0..ctmpMaxAttDataLen       -1] of  char;
//    ctmpPwdString          = array[0..ctmpMaxAgentId-1] of           char; //2011.03.03 삭제
    ctmpEmployeeString	   = array[0..ctmpMaxEmployeeLen      -1] of  char;   //* added by K.CH.S 2008.01.29 nexus */
    ctmpString30	   = array[0..ctmpMaxLen30            -1] of  char;   //   /* added by K.CH.S 2008.01.29 nexuscube */
    ctmpString20	   = array[0..ctmpMaxLen20            -1] of  char;   //   /* added by K.CH.S 2008.01.29 nexuscube */
    ctmpAgentPath          = array[0..ctmpMaxAttDataLen       -1] of  char;
