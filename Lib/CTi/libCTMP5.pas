...
uses
  Windows, SysUtils, Classes, defCTMP5, constCTMP5, Forms, ComCtrls,  AdvOfficeStatusBar,
  varCTMP5, Dialogs;
type
...
{$ENDIF}    
    ...
    procedure Finalize;
    procedure SetStatus(tForm: TCustomForm; tBar: TAdvOfficeStatusBar; Index: Integer; Flag: Boolean=True);
...
//    function GetAgentStatusEx(var AgentRec: TAgentRec; AID: ctmpDeviceString; DeviceNO: ctmpDeviceString ): Integer;
    ...
    function SetAgentStatus(aMode: ctmpAgentMode_Def; aReasoncode: ctmpINT = 0): Integer; overload;
    function SetAgentStatus(aMode: ctmpAgentMode_Def; DN: AnsiString; aReasoncode: ctmpINT = 0): Integer; overload;
    function ConnectionServer : Integer;
    function CloseServer(ID: ctmpGateID): Integer;

    procedure fCTIGetErrorMsg(ErrorCode: ctmpError_Def; Msg: AnsiString = ''; Flag: Boolean = False);
    function fCtiLogin(sCtiID, sDevice: AnsiString): Integer;
    function fCtiLogOut: Integer;
    function fCtiMakeCall(sDial: AnsiString; Msg: AnsiString=''): Integer;
    function fCtiPWDConfirm(Msg: AnsiString=''): Integer;
    function fCtiSingleTransferCall(sDial: AnsiString): Integer;
    function fCtiAnswer: Integer;
    function fCtiDisconnect: Integer;
    function fCtiHold: Integer;
    function fCtiRetrieve: Integer;
    function fCtiReconnectCall: Integer;
    function fCtiWaitTimeGet(queueDN: ctmpDataString): Integer;
  end;
...
begin
  ...
  StrCopy(AgentData, PAnsiChar('000'));    //20110609
  StrCopy(AgentGroup, PAnsiChar('000'));

  if TmrRec.RemoteLogin then
    StrCopy(ServerName, PAnsiChar('192.168.1.153')) //192.168.1.151
  else
    StrCopy(ServerName, PAnsiChar(CTIServRec.CTIIP));

{$IFDEF avaya_ctmp5}
  if CTIServRec.HAUSE = 1 then  //cti서버 이중화를 한다면
  begin
    StrCopy(ServerName2, PAnsiChar(CTIServRec.CTIIP2));
  end;
{$ENDIF}
...
//  StrCopy(openData.deviceDn, DeviceNumber);

  if CTIServRec.CTIType = ctmpV_None then
  begin
{$IFDEF cti_ccc}
    openData.APIextensions  := ctmpV_CSTA2;
{$ELSE}
    openData.APIextensions  := ctmpV_ASAI;
{$ENDIF}
  end
  else
  begin
    openData.APIextensions  := CTIServRec.CTIType;
{$IFDEF cti_ccc}
    if CTIServRec.CTIType <> ctmpV_CSTA2 then openData.APIextensions  := ctmpV_CSTA2;
{$ENDIF}
  end;
...
var
  AgentRec: TAgentRec;
  r,iReasoncode, iReasonResult: Integer;
begin

  Debug.Write(1, 99991, 'ConnectionServer Started!');
  Initialize;
  Debug.Write(1, 99991, 'ConnectionServer Info : ' +AppName+'   '+ StrPas(ServerName) + ' : ' + inttostr(PortNo));
{$IFDEF avaya_ctmp5}
...
{}
  ...
  if (AgentRec.eAgentMode <> ctmpV_AgentLogout) and (AgentRec.eAgentDN > 0) then
  begin
    if (MessageBox(0, PChar('다른 상담원이 아이디['+(sAgentID)+']를 사용중에 있습니다.'+#13+#10+''+#13+#10+
                      ...
                      '로그아웃 후 재로그인 하시겠습니까?'), 'CTMP', MB_ICONQUESTION or MB_OKCANCEL) = IDOK) then

    Result := SetAgentStatus(ctmpV_AgentLogout, IntToStr(AgentRec.eAgentDN));
    Debug.Write(1, 99991, 'SetAgentStatus ctmpV_AgentLogout[로그아웃 후 재로그인],  Result='+IntToStr(Result));
    if Result <> ctmpNormal then
    begin
      fCTIGetErrorMsg( Result, 'ConnectionServer - SetAgentStatus' );

      //2021.05.15
      //EmgLogout 함수 호출시 API 내부적으로
      //SetFeatureAgentStatus (mode : loggedoff, reasoncode : 100) 요청
      sleep(1000);
      iReasoncode := 100;
      iReasonResult := SetAgentStatus(ctmpV_AgentLogout, IntToStr(AgentRec.eAgentDN),iReasoncode);
      Debug.Write(1, 99991, 'SetAgentStatus ctmpV_AgentLogout[로그아웃 후 재로그인-emplogout],  Result='+IntToStr(Result)+', iReasoncode= '+IntToStr(iReasoncode));
      sleep(1000);
      if iReasonResult <> ctmpNormal then
      begin
        CloseServer(GateID);
        exit;
      end;
  ...
  begin
    // 로그아웃 되어있어도 한번 로그아웃을 던진다.
    Result := SetAgentStatus(ctmpV_AgentLogout);
    Debug.Write(1, 99991, 'SetAgentStatus ctmpV_AgentLogout,  Result='+IntToStr(Result));
  end;
...
{}
  Initialize;
{$IFDEF avaya_ctmp5}
  if CTIServRec.HAUSE = 0 then //단일화 서버 클로즈
  begin
...
begin
  Debug.Write(1, 99991, 'SetAgentStatus '#13#10 + 'DN: ' + DeviceNumber +
                                         #13#10 + 'aMode :' + inttostr(amode) +
                                         #13#10 + 'aReasoncode :' + inttostr(aReasoncode) +
                                         #13#10 + 'agentID : ' + agentID );
...
function TCTMP.SetAgentStatus(aMode: ctmpAgentMode_Def; DN: AnsiString; aReasoncode: ctmpINT = 0): Integer;
var
  arrDN: ctmpDeviceString;
begin
  Debug.Write(1, 99991, 'SetAgentStatus '#13#10 + 'DN: ' + DeviceNumber +
                                         #13#10 + 'aMode :' + inttostr(amode) +
                                         #13#10 + 'aReasoncode :' + inttostr(aReasoncode) +
  ...
  if DN = '' then
  begin
    StrCopy( arrDN, DeviceNumber);
  end
  else
  begin
    StrCopy( arrDN, PAnsiChar(DN));
  end;

  Result := nxcapiSetFeatureAgentStatus(GateID, Invok, arrDN, aMode,
                                        AgentId, AgentData, AgentGroup,
                                        aReasoncode, 0, privateData);
  WorkStateCD := aReasoncode;
end;
...
begin
  ...
  else StrCopy(Device, PAnsiChar(IntToStr(DeviceNo)));
...
begin
  ...
  begin
    ...
    begin
      Result := nxcapiQueryAgentStatusEx(GateID, Invok, '',  DeviceNO,
                                       eAgentID, eAgentMode, eAgentDN, eAgentblendMode, eAgentBlockMode, eAgentTime, eAgentType);
    end;
  end;
  ...
  Debug.Write(1, 99991, 'GetAgentStatus,  AgentRec.AgentDN='+IntToStr(AgentRec.eAgentDN));
end;
...
procedure TCTMP.SetStatus(tForm: TCustomForm; tBar: TAdvOfficeStatusBar; Index: Integer; Flag: Boolean=True);
var
  nRtn : Integer;
...
{$IFDEF cti_NotReady_fis}
    // 2024.12.13 한국재정정보원(e나라,디브레인용)
...
{$ENDIF}
  ...
  begin
    with tForm do
      tBar.Panels[Index].Text := workStr+FormatDateTime('"["HH:NN:SS"]"', Now()-Statetime);
  end;
end;

function TCTMP.fCtiLogin(sCtiID, sDevice: AnsiString): Integer;
var
  nReturn: Integer;
begin
  Debug.Write(1, 31100, 'fCtiLogin :"Start", sCtiID :"'+sCtiID+'", sDevice :"'+ String(sDevice)+'"');

  if (sCtiID = '') or (sDevice = '') then
  begin
     nReturn := -1;
     exit;
  end;
  ...
  begin
    ...
    StrCopy(deviceDn, PAnsiChar(sDevice));
  end;
  StrCopy(DeviceNumber, PAnsiChar(sDevice));
  StrCopy(AgentId,      PAnsiChar(sCtiID));
  StrCopy(NetWorkType,  PAnsiChar(ctmpNetWorkType));
  StrCopy(AppName,      PAnsiChar(ctmpAppName));
  DefPbx := ctmpDefPbx;
  Invok  := ctmpInvok;
...
//  iAgentID := sCtiID;
  StrLCopy(sAgentID, PAnsiChar(@sCtiID[1]), high(sAgentID));
  iDeviceNumber := ctmpINT(StrToInt(String(sDevice)));
...
function TCTMP.fCtiMakeCall(sDial: AnsiString; Msg: AnsiString=''): Integer;
var
  nReturn: Integer;
begin
  Debug.Write(1, 31100, 'fCtiMakeCall :"Start", sDial="'+String(sDial)+'", Msg='+String(Msg));
  ...
  strCopy(CalledNumber, PAnsiChar(sDial)); //대상전화번호
  StrCopy(Account, PAnsiChar(''));
  StrCopy(AuthCode, PAnsiChar(''));
  StrCopy(UUI, PAnsiChar(''));

  with UEI do
  begin
    UEILen := Length(Msg);
    strCopy(UEIData, PAnsiChar(Msg));
  end;
...
function TCTMP.fCtiPWDConfirm(Msg: AnsiString =''): Integer;
var
  nReturn: Integer;
begin
  Debug.Write(1, 31100, 'fCtiPWDConfirm :"Start", Msg='+String(Msg));
...
function TCTMP.fCtiSingleTransferCall(sDial: AnsiString): Integer;
var
  nReturn: Integer;
  nNewCall        :   u_int;
begin
  Debug.Write(1, 31100, 'fCtiSingleTransferCall :"Start", sDevice :"'+String(sDial)+'"');
...
function TCTMP.fCtiReconnectCall: Integer;
var
  nReturn: Integer;
begin
  Debug.Write(1, 31100, 'fCtiReconnectCall :"Start"');
  nReturn := 9999;
  case GetAgentState of
    ctmpV_AgentOtherWork :begin
{$IFDEF cti_ccc}
      nReturn := nxcapiReconnectCall(GateID, Invok, HeldCall, ActCall, privateData);
{$ELSE}
      nReturn := nxcapiReconnectCall(GateID, Invok, HeldCall, ActCall, privateData);
//      nReturn := nxcapiReconnectCall(GateID, Invok, HeldCall, NewCall2, privateData);
{$ENDIF}

      if nReturn <> ctmpNormal then fCTIGetErrorMsg( nReturn , 'fCtiReconnectCall - ctmpReconnectCall' );
    end;
  end;
  Result := nReturn;
  Debug.Write(1, 31100, 'fCtiReconnectCall :"End", Return :"'+IntToStr(nReturn)+'"');
end;

function TCTMP.fCtiWaitTimeGet(queueDN: ctmpDataString): Integer;
var
  nReturn: Integer;
  waitTime  : ctmpINT;
  waitCount : ctmpSHORT;
begin
  Debug.Write(1, 31100, 'fCtiWaitTimeGet :"Start"');

  nReturn := nxcapiWaitTimeGet(GateID, Invok, 0, queueDN, waitTime, waitCount );
  if nReturn <> ctmpNormal then
  begin
    fCTIGetErrorMsg( nReturn , 'fCtiWaitTimeGet - nxcapiWaitTimeGet' );
    Result := -1;
  end
  else  Result := waitCount;
  Debug.Write(1, 31100, 'fCtiWaitTimeGet :"End", nReturn :"'+IntToStr(nReturn)+'"');
end;

procedure TCTMP.fCTIGetErrorMsg(ErrorCode: ctmpError_Def; Msg: AnsiString = ''; Flag : Boolean = False);
var
  ErrorMsg: String;
begin
  ...
  if Msg = '' then
    ...
    ErrorMsg := String(Msg) + ':' + GetErrorMessage(ErrorCode);
