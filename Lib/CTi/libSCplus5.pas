...
//    function CTI_Alternate : Integer; stdcall; External DLLNAME;
    function fCtiConference : Integer; 
...
var
  nReturn: Integer;
begin
  Debug.Write(1, 31100, 'fCtiConference :"Start"');
  nReturn := 999;
  case fCTIGetAgentState of
    STATE_C :begin
      nReturn := CTI_Conference();
      if nReturn <> 0 then fCtiGetErrorMsg;
      fCTIGetAgentState;
    end;
  end;
  Result := nReturn;
  Debug.Write(1, 31100, 'fCtiConference :"End", Return :"'+IntToStr(nReturn)+'"');
end;
    ...
    STATE_I :begin
      ...
      begin
        ...
        begin
          ...
          with OutBoundRec do
          begin
            Debug.Write(1, 31100, 'fCtiGetOutBoundScrData, Data'+Str);
            CampSeqNO  := Trim(Copy(Str, 1, 8));    //캠페인 일련번호                     1
            CampaignID := Trim(Copy(Str, 9, 12));   //캠페인 번호                         2
            CallMode   := Trim(Copy(Str, 21, 2));   //콜 모드                             3
            CustID     := Trim(Copy(Str, 23, 24));  //고객ID       WFM상 자릿수(11)       4
            CustTel    := Trim(Copy(Str, 47, 24));  //고객전화번호 WFM상 자릿수(12)       5
            CustInfo1  := Trim(Copy(Str, 71, 32));  //고객정보1    WFM상 자릿수(12)       6
            CustInfo2  := Trim(Copy(Str, 103, 32)); //고객정보2    WFM상 자릿수(15)       7
            CustInfo3  := Trim(Copy(Str, 135, 32)); //고객정보3    WFM상 자릿수(15)       8
            CustInfo4  := Trim(Copy(Str, 167, 32)); //고객정보4    WFM상 자릿수(15)       9
            CustInfo5  := Trim(Copy(Str, 199, 32)); //고객정보5    WFM상 자릿수(15)       10
            CustInfo6  := Trim(Copy(Str, 231, 32)); //고객정보6                           11
            CallResult := Trim(Copy(Str, 263, 2));  //전화결과                            12
            ReservedGB := Trim(Copy(Str, 265, 1));  //예약호 여부                         13
            CallLogID  := Trim(Copy(Str, 266, 16)); //콜 로그ID                           14
            ReservedDT := Trim(Copy(Str, 282, 14)); //예약날짜                            15
            Debug.Write(1, 31100, ' CampSeqNO  ='+CampSeqNO  +#13#10+
                                  ',CampaignID ='+CampaignID +#13#10+
                                  ',CallMode   ='+CallMode   +#13#10+
                                  ',CustID     ='+CustID     +#13#10+
                                  ',CustTel    ='+CustTel    +#13#10+
                                  ',CustInfo1  ='+CustInfo1  +#13#10+
                                  ',CustInfo2  ='+CustInfo2  +#13#10+
                                  ',CustInfo3  ='+CustInfo3  +#13#10+
                                  ',CustInfo4  ='+CustInfo4  +#13#10+
                                  ',CustInfo5  ='+CustInfo5  +#13#10+
                                  ',CustInfo6  ='+CustInfo6  +#13#10+
                                  ',CallResult ='+CallResult +#13#10+
                                  ',ReservedGB ='+ReservedGB +#13#10+
                                  ',CallLogID  ='+CallLogID  +#13#10+
...
begin
  ...
  case fCTIGetAgentState of
    STATE_I :begin
      ...
      try
        GetMem( Data_Arr, 65536 );
        StrPCopy( Data_Arr , sData );
        nReturn := CTI_CampaignLogin(nOption, PChar(sAgentID), Data_Arr);
      finally
        FreeMem( Data_Arr );
      end;
      if nReturn <> 0 then fCTIGetErrorMsg;
