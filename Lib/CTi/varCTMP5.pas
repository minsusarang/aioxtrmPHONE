...
{$IFDEF avaya_ctmp5}
    CTIIP   : AnsiString;
    CTIIP2  : AnsiString; // pjw ctmp5 서버이중화 때문에 추가 20110518
    CTIPort : AnsiString;
    CTIPort2 : AnsiString; // 이중화 포트를 위해 작성하였으나 실제 사용하지는 않음.!!
...
{$ENDIF}
  ...
  TTmrRec = record
    ...
    WEB_ID      : AnsiString;  //WEB_ID 아이디
    WEB_PW      : AnsiString;  //WEB_PW 아이디
  end;
  ...
  TCustRec = record
    ...
    JuID      : AnsiString; //주민번호 2011.06.01 msPark.
    CustNum   : AnsiString;  //고객사회원번호
    CampID    : AnsiString;  //캠페인ID
    CompanyCD : AnsiString;  //회사구분코드
    CompanyNM : AnsiString;  //회사명

    CardPass  : string;  //실명인증코드
    Remark    : string;  //비고
    CAMP_MGcode : AnsiString;  //캠프코드
    EXEC_DT     : AnsiString;  //캠프코드
    EXEC_NUMBER : AnsiString;  //캠프코드
  ...
  TCallTransListRec = record //호전환리스트
    IsCallTransList        : Boolean;
    CallTransListCount     : Integer;
    CallTransNameList      : array of AnsiString;
    CallTransList          : array of AnsiString;
  end;
  ...
  TReservSetRec = record
    ...
    ReservTmrID  : String;       //예약담담자처리 담당자..
  end;
...
{}
    ...
    INOUTGB    : AnsiString;       //인바운드/아웃바운드 구분 [I:인바운드|0:아웃바운드]
    ...
    CALL_REMARK : AnsiString;

    SCENARIO_SEQ: AnsiString;
    COMPANY_CD  : AnsiString;
    PRODUCT_CD  : AnsiString;
  end;
...
var
  ...
  CallTransListRec: TCallTransListRec; //호전환 리스트
