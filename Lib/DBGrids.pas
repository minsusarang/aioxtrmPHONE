...
{ modifed by mparam }
  TFieldSortOrder = ( soNone , soAscending , soDescending );
{ modifed by mparam }
...
{ modifed by mparam }
    property  SortOrder: TFieldSortOrder read FSortOrder write FSortOrder;
...
{ modifed by mparam }
  TDBGridCheckBoxClickEvent = procedure (Sender: TObject; Column: TColumn) of object;
{ modifed by mparam }
...
{ modifed by mparam }
    ...
    FShowColorDiv: Boolean;
    FShowRowNum: Boolean;
    FRowNumWidth: Integer;
    ...
    FPressedPoint: TPoint;
//    FPressedCol: TColumn;
    FPressedCol: Integer;
    FPressed: Boolean;
    FTracking: Boolean;

    FOnCheckBoxClick: TDBGridCheckBoxClickEvent;
{ modifed by mparam }
...
{ modifed by mparam }
    function PtInCheckBox(X,Y: Integer; var MasterCol: TColumn): Boolean;
...
//    function DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean; override;
    ...
    function CalcCheckBoxRect(Col: TColumn; ARow: Integer): TRect;

    procedure SetFixedCols(Value: Integer);
    function GetFixedCols: Integer;

    procedure SetShowRowNum(Value: Boolean);
    procedure SetRowNumWidth(Value: Integer);

    procedure SetCheckFields(Value: String);
...
{ modifed by mparam }
    function GetCheckedField: String;
    procedure CheckAll;
    procedure CheckNotAll;
{ modifed by mparam }
    property EditorMode;
...
{ modifed by mparam }
    ...
    property FixedFields: Integer read GetFixedCols write SetFixedCols default 0;

    property ShowColorDiv: Boolean read FShowColorDiv write FShowColorDiv;
    property ShowRowNum: Boolean read FShowRowNum write SetShowRowNum;
    property RowNumWidth: Integer read FRowNumWidth write SetRowNumWidth;

    property WheelEnable: Boolean read FWheelEnable write FWheelEnable;
    property WheelDelta: Integer read FWheelDelta write FWheelDelta;

    property CheckFields: String read FCheckFields write SetCheckFields;
    property ShowCheckBox: Boolean read FShowCheckBox write SetShowCheckBox;
    property Checked: Boolean read FChecked write SetChecked;

    property OnCheckBoxClick: TDBGridCheckBoxClickEvent read FOnCheckBoxClick write FOnCheckBoxClick;
{ modifed by mparam }
  end;
...
uses Math, DBConsts, VDBConsts, Dialogs, RTLConsts, Themes, UxTheme;
...
procedure TColumn.SetChecked(Value: Boolean);
begin
  ...
  if Assigned( TDBGridColumns(Collection).Grid ) then
    TDBGridColumns(Collection).Grid.InvalidateTitles;
end;
...
procedure TDBGridColumns.ClearSortOrder;
var
  I: Integer;
begin
  for I := 0 to Count-1 do
...
{  modified by mparam }
  ...
  FFixedOffset := FIndicatorOffset;

  FFixedCols := 0;

  FShowColorDiv := False;
  FShowRowNum := False;
  FRowNumWidth := DefRowNumWidth;
...
begin
  ...
  try
    for I := FFixedOffset to ColCount - 1 do
      FColumns[I - FFixedOffset].Width := ColWidths[I];
  finally
    EndLayout;
  end;
...
begin
  ...
  if TMessage(Msg).Msg = wm_RButtonUp then
    ...
    begin
      ...
      if (Cell.X < inherited FixedCols) or (Cell.Y < 0) then Exit;
...
var
  ...
  begin
    ...
    if FShowCheckBox and ( Column.ShowCheckBox ) then
      CheckBoxRect := CalcCheckBoxRect( Column , ARow )
    else
      CheckBoxRect := Rect( 0 , 0 , 0 , 0 );

    if FShowCheckBox and ( Column.ShowCheckBox ) and ( MasterCol.Title.Alignment = taLeftJustify ) then
      Canvas.FillRect(ARect);
    ...
    if FShowCheckBox and ( Column.ShowCheckBox ) and ( MasterCol.Title.Alignment = taLeftJustify ) then
      TextRect.Left := TextRect.Left + ( CheckBoxRect.Right - CheckBoxRect.Left ) + 2;
    ...
    if Column.Checked then
      DrawFrameControl( Canvas.Handle , CheckBoxRect , DFC_BUTTON , DFCS_BUTTONCHECK or DFCS_FLAT or DFCS_CHECKED )
    else
      DrawFrameControl( Canvas.Handle , CheckBoxRect , DFC_BUTTON , DFCS_BUTTONCHECK or DFCS_FLAT );
...
// Draw Indicator Image
    if ( FIndicatorOffset > 0 ) and ( RawCol = 0 ) then
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          CheckBoxRect := CalcCheckBoxRect( nil , ARow );
          if FChecked then
            DrawFrameControl( Canvas.Handle , CheckBoxRect , DFC_BUTTON , DFCS_BUTTONCHECK or DFCS_FLAT or DFCS_CHECKED )
          else
            DrawFrameControl( Canvas.Handle , CheckBoxRect , DFC_BUTTON , DFCS_BUTTONCHECK or DFCS_FLAT );
        ...
        begin
          ...
          Canvas.MoveTo( ARect.Left-3 , ARect.Top-1 );
          Canvas.LineTo( ARect.Right+3 , ARect.Bottom+1 );
        end;
      end
      else
      if ARow >= 0 then
      begin
        ...
        begin
          ...
          try
            FDataLink.ActiveRecord := ARow;
            WriteText( Canvas , ARect , FrameOffs , FrameOffs , IntToStr(FDataLink.DataSet.RecNo) , taRightJustify , False );
          finally
            FDataLink.ActiveRecord := OldActive;
          end;
        end;
...
// FixedCols 가 있어도 데이터 부분은 보통 색깔로 찍자.
    if ( ARow >= 0 ) then
    begin
      if (gdFixed in AState) and ([dgRowLines, dgColLines] * Options =
        [dgRowLines, dgColLines]) then
    ...
    begin
      ...
      try
        ...
        if Highlight and DefaultDrawing then
          DrawCellHighlight(ARect, AState, ACol, ARow);

        if FShowColorDiv then
        begin
          if (((FDataLink.DataSet.RecNo-1) div 5) mod 2) =0 then Brush.Color := $00FFEFEC;
        end;

// Highlight 부분은 설정한 색깔로.
        if Highlight or ( (ACol < FixedCols ) and (ARow = OldActive) ) then
        begin
          Brush.Color := FSelColor;
          Font.Color := FSelFontColor;
        end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          NextRow(False);
          ACol := inherited FixedCols;
        end
        else if ACol < inherited FixedCols then
        begin
          PriorRow(False);
          ACol := ColCount - FFixedOffset;
        end;
  ...
  with FDatalink.DataSet do
    ...
    begin
      ...
      case Key of
        ...
        VK_LEFT: MoveCol(FFixedOffset, 1);
...
begin
  ...
  if Assigned(DataLink.DataSet) and (not DataLink.DataSet.IsSequenced) then
    FShowRowNum := False;

  FFixedOffset := FIndicatorOffset;
  if FShowRowNum then Inc(FFixedOffset);

  ColCount := FColumns.Count + FFixedOffset;
  inherited FixedCols := FFixedOffset + FFixedCols;
  FTitleOffset := 0;
  if dgTitles in Options then
  begin
    FTitleOffset := 1;
...
begin
  ...
  if (DragKind = dkDock) and (Cell.X < FFixedOffset) and
  ...
  if PtInCheckBox( X , Y , MasterCol ) then
  begin
    if Assigned( MasterCol ) then
      MasterCol.Checked := not MasterCol.Checked
    else
    ...
    ReleaseCapture;
{
    ...
    if Assigned( FOnCheckBoxClick ) then
      FOnCheckBoxClick( Self , MasterCol );
...
(*
  ...
  begin
    ...
    DoClick := PtInRect(Rect(0, 0, ClientWidth, ClientHeight), Point(X, Y))
      and (Cell.Y < FTitleOffset) and (Cell.X = FPressedCol);
    StopTracking;
...
*)
  ...
  if PtInCheckBox( FPressedPoint.X , FPressedPoint.Y , MasterCol ) then
    Exit;

  Cell := MouseCoord(X,Y);
  if (Button = mbLeft) and (Cell.X >= FFixedOffset) and (Cell.Y >= 0) then
...
begin
  ...
  if RawCol >= ColCount then
    RawCol := ColCount - 1;
  if RawCol < inherited FixedCols then RawCol := inherited FixedCols;
  if Direction <> 0 then
  begin
    while (RawCol < ColCount) and (RawCol >= inherited FixedCols) and
      (ColWidths[RawCol] <= 0) do
      Inc(RawCol, Direction);
    if (RawCol >= ColCount) or (RawCol < inherited FixedCols) then Exit;
  end;
  OldCol := Col;
...
begin
  ...
  with FColumns[I] do
  begin
    TabStops[I + FFixedOffset] := Showing and not ReadOnly and DataLink.Active and
      Assigned(Field) and not (Field.FieldKind = fkCalculated) and not ReadOnlyField(Field);
    ColWidths[I + FFixedOffset] := Width;
  end;
  if (dgIndicator in Options) then
    ColWidths[0] := IndicatorWidth;
  if FShowRowNum then
    ColWidths[FFixedOffset-1] := FRowNumWidth;
...
begin
  ...
  begin
    ...
    if dgRowLines in ChangedOptions then UpdateRowCount;
  end;
end;
...
function TCustomDBGrid.GetFixedCols: Integer;
begin
  if DataLink.Active then Result := inherited FixedCols - FFixedOffset
  else Result := FFixedCols;
end;

procedure TCustomDBGrid.SetFixedCols(Value: Integer);
var
  FixCount, I: Integer;
begin
  FixCount := Max(Value, 0) + FFixedOffset;
  if DataLink.Active and not (csLoading in ComponentState) and
    (ColCount > FFixedOffset + 1) then
  begin
  ...
  FFixedCols := FixCount - FFixedOffset;
end;

{
function TCustomDBGrid.DoMouseWheelDown(Shift: TShiftState;
  MousePos: TPoint): Boolean;
begin
  Result := False;
  if FWheelEnable then
  begin
    if not FDatalink.Active then
      Exit;

//    for I := 1 to FWheelDelta do
//      FDatalink.DataSet.MoveBy( 1 );
    FDatalink.DataSet.MoveBy( FWheelDelta );
    Result := True;
  end
  else
...
function TCustomDBGrid.DoMouseWheelUp(Shift: TShiftState;
  MousePos: TPoint): Boolean;
begin
  Result := False;
  if FWheelEnable then
...
procedure TCustomDBGrid.SetRowNumWidth(Value: Integer);
begin
  if FRowNumWidth = Value then
    Exit;
...
procedure TCustomDBGrid.SetShowRowNum(Value: Boolean);
begin
  if FShowRowNum = Value then
    Exit;
...
function TCustomDBGrid.CalcCheckBoxRect(Col: TColumn; ARow: Integer): TRect;
var
  ColRect: TRect;
  MasterCol: TColumn;
begin
  ...
  begin
    ColRect := CellRect( FIndicatorOffset , 0 );

    if ( ColRect.Right - ColRect.Left ) < RowHeights[0] then
      ColRect.Right := ColRect.Left + RowHeights[0];

    InflateRect( ColRect , -1 , -1 );

    if ( ColRect.Right - ColRect.Left ) > ( ColRect.Bottom - ColRect.Top ) then
      InflateRect( ColRect , ((ColRect.Right - ColRect.Left) - (ColRect.Bottom - ColRect.Top)) div 2 , 0 )
    else
      InflateRect( ColRect , 0 , ((ColRect.Bottom - ColRect.Top) - (ColRect.Right - ColRect.Left)) div 2 );
    Result := ColRect;
  end
  else
  begin
    if Col.ShowCheckBox then
    begin
      ColRect := CalcTitleRect( Col , ARow , MasterCol );
...
function TCustomDBGrid.PtInCheckBox(X, Y: Integer;
  var MasterCol: TColumn): Boolean;
var
  Cell: TGridCoord;
  R: TRect;
begin
  ...
  Result := False;
  Cell := MouseCoord(X,Y);
  if (Cell.Y < FTitleOffset) and FDatalink.Active then
  begin
    if ( FShowRowNum ) and ( Cell.X = FFixedOffset - 1 ) then
      Result := FShowCheckBox
    else if (Cell.X >= FFixedOffset) and (RawToDataColumn(Cell.X) < Columns.Count) then
    begin
      R := CalcTitleRect(Columns[RawToDataColumn(Cell.X)], Cell.Y, MasterCol);
      R := CalcCheckBoxRect( Columns[RawToDataColumn(Cell.X)] , Cell.Y );
//      if not Assigned( MasterCol ) or not MasterCol.ShowCheckBox then
//        Exit;
      Result := FShowCheckBox and MasterCol.ShowCheckBox and PtInRect( R , Point( X , Y ) );
    end;
  end;
end;

procedure GridInvalidateRow(Grid: TCustomDBGrid; Row: Longint);
var
  I: Longint;
begin
...
procedure TCustomDBGrid.TrackButton(X, Y: Integer);
var
  Cell: TGridCoord;
  NewPressed: Boolean;
  I: Integer;
...
//  if (Cell.X >= FFixedOffset) and (RawToDataColumn(Cell.X) < Columns.Count) then
//    CalcTitleRect( Columns[RawToDataColumn(Cell.X)] , Cell.Y , MasterCol );

  NewPressed := PtInRect(Rect(0, 0, ClientWidth, ClientHeight), Point(X, Y)) and
    ( FPressedCol = Cell.X ) and ( Cell.Y < FTitleOffset );
...
procedure TCustomDBGrid.StopTracking;
begin
  if FTracking then
  begin
    TrackButton(-1, -1);
...
procedure TCustomDBGrid.SetChecked(Value: Boolean);
begin
  if FChecked = Value then
    Exit;
...
procedure TCustomDBGrid.SetCheckFields(Value: String);
var
  GroupFields: TStringList;

  I: Integer;

  function FindField( FieldName: String ): TColumn;
  var
    J: Integer;
  begin
    Result := nil;
    for J := 0 to Columns.Count-1 do
      if CompareText( Columns.Items[J].FieldName , FieldName ) = 0 then
      begin
        Result := Columns.Items[J];
        Exit;
      end;
  end;

begin
  if FCheckFields = Value then
    Exit;
  ...
  try
    GroupFields.CommaText := StringReplace( Value , ';' , ',' , [rfReplaceAll] );

    for I := 0 to Columns.Count-1 do
      Columns.Items[I].ShowCheckBox := False;

    for I := 0 to GroupFields.Count-1 do
    begin
      if Assigned( FindField( GroupFields[I] ) ) then
      begin
        FindField( GroupFields[I] ).ShowCheckBox := True;
        FindField( GroupFields[I] ).Checked := True;
      end;
    end;
  finally
    FreeAndNil( GroupFields );
  end;

  ShowCheckBox := True;
end;

procedure TCustomDBGrid.SetShowCheckBox(Value: Boolean);
begin
...
function TCustomDBGrid.GetCheckedField: String;
var
  I: Integer;
begin
  Result := '';
  ...
  for I := 0 to Columns.Count-1 do
    if Columns.Items[I].Checked then
      Result := Result + Columns.Items[I].FieldName + ';';

  if Result <> '' then
    Result := Copy( Result , 1 , Length(Result)-1 );
end;

procedure TCustomDBGrid.CheckAll;
var
  I: Integer;
begin
  FChecked := True;
  for I := 0 to Columns.Count-1 do
    if Columns.Items[I].ShowCheckBox then
      Columns.Items[I].Checked := True;

  InvalidateTitles;
end;

procedure TCustomDBGrid.CheckNotAll;
var
  I: Integer;
begin
  FChecked := True;
