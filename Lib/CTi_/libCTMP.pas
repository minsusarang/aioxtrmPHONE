...
uses
  Windows, SysUtils, Classes, varCTMP, defCTMP, constCTMP, Forms, ComCtrls, Dialogs;
...
type
  ...
  public
    ...
    function GetAgentStatusEx(var AgentRec: TAgentRec; AID: ctmpINT; DeviceNO: ctmpINT = 0): Integer;
...
begin
  Initialize;
  Result := ctmpOpenServer(GateID, Invok, OpenData, ServerName, PortNo,
  ...
  Result := ctmpMonitorStart(GateID, Invok);
  if Result <> ctmpNormal then
  begin
    fCTIGetErrorMsg( Result, 'ConnectionServer - ctmpMonitorStart' );
    ctmpCloseServer(GateID, Invok);
    Exit;
  end;


  Result := GetAgentStatusEx(AgentRec, iAgentID);
  if AgentRec.eAgentMode <> ctmpV_AgentLogout then
  begin
    if (MessageBox(0, PChar('다른 상담원이 아이디['+IntToStr(iAgentID)+']를 사용중에 있습니다.'+#13+#10+''+#13+#10+
...
begin
  Initialize;
  r := ctmpMonitorStop(ID, Invok);
  r := ctmpCloseServer(ID, Invok);
  Result := r;
end;

function TCTMP.SetAgentStatus(aMode: ctmpAgentMode_Def; aReasoncode: ctmpINT = 0): Integer;
begin
  Result := ctmpSetFeatureAgentStatus(GateID, Invok, DeviceNumber, aMode,
                                      AgentId, AgentData, AgentGroup,
                                      aReasoncode, ctmp_AUX_WORK, privateData);
...
begin
  ...
  Result := ctmpQueryAgentStatus(GateID, Invok, Device, sAgentMode, AgentID, sReasonCode, sPrivateData);
...
//  Result := ctmpQueryAgentStatusEx(GateID, Invok, iAgentID, iDialNumber,
  with AgentRec do
    Result := ctmpQueryAgentStatusEx(GateID, Invok, AID, DeviceNO,
  ...
  Debug.Write(1, 99991, 'GetAgentStatus,  AgentRec.AgentID='+IntToStr(AgentRec.eAgentID));
end;
...
begin
  ...
  if GateID > 0 then
  begin
    nRtn := GetAgentStatusEx(AgentRec, iAgentID);
...
begin
  ...
  //서버Open 시에만 적용됨
  case gAgentMode of
  ctmpV_AgentLogin         : workStr := '로그인';
  ctmpV_AgentLogout        : workStr := '로그아웃';
  ...
  ctmpV_AgentReady         : workStr := '대기';
  ctmpV_AgentOtherWork     : workStr := '통화중';
  ctmpV_AgentAfterCallWork : workStr := '후처리';
  end;
...
begin
  ...
  nReturn := ctmpMakeCall(GateID, Invok, DeviceNumber, CalledNumber, UUI, Account, AuthCode,
                       NewCall,
                       UEI,
                       CI,
                       privateData);
  ...
  begin
    fCTIGetErrorMsg( Result, 'fCtiMakeCall - ctmpMakeCall' );
//    Exit;
  end;
...
//  NewCall.devIDType := STATIC_ID;
  ...
  nReturn := ctmpSingleStepTransfer( GateID,
                                              Invok,
                                              ActCall,
                                              CalledNumber,
                                              ctmpV_On,
...
begin
  ...
  nReturn := ctmpAnswerCall(GateID, Invok, ActCall, privateData);
...
begin
  ...
  nReturn := ctmpClearConnection(GateID, Invok, ActCall, PrivateData);
...
begin
  ...
  case GetAgentState of
    ctmpV_AgentOtherWork :begin
      nReturn := ctmpHoldCall(GateID, Invok, ActCall, Byte('0'), privateData);
...
//  case GetAgentState of
//    ctmpV_AgentOtherWork :begin
      nReturn := ctmpRetrieveCall(GateID, Invok, HeldCall, privateData);
