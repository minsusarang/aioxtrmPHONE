...
var
    ...
    i               : Integer;
begin
    Debug.Write(1, 99999, 'SeedEncrypt');
    L0              := $0;
    L1              := $0;
    R0              := $0;
    R1              := $0;
...
begin
    ...
    A       := (A shl 8) xor ((pbUserKey[1] and $000000ff));
    A       := (A shl 8) xor ((pbUserKey[2] and $000000ff));
    A       := (A shl 8) xor ((pbUserKey[3] and $000000ff));

    B       := pbUserKey[4] and $000000ff;
    B       := (B shl 8) xor ((pbUserKey[5] and $000000ff));
    B       := (B shl 8) xor ((pbUserKey[6] and $000000ff));
    B       := (B shl 8) xor ((pbUserKey[7] and $000000ff));

    C       := pbUserKey[8] and $000000ff;
    C       := (C shl 8) xor ((pbUserKey[9] and $000000ff));
    C       := (C shl 8) xor ((pbUserKey[10] and $000000ff));
    C       := (C shl 8) xor ((pbUserKey[11] and $000000ff));

    D       := pbUserKey[12] and $000000ff;
    D       := (D shl 8) xor ((pbUserKey[13] and $000000ff));
    D       := (D shl 8) xor ((pbUserKey[14] and $000000ff));
    D       := (D shl 8) xor ((pbUserKey[15] and $000000ff));
